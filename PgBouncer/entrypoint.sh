...
###############################################################################
## Sydecar specefic improvements to pull DB String details from Secrets Manager
## at runtime.
###############################################################################
# Check if "USE_SECRET_MANAGER" is set and if so extract secrets
# and form a DB String of the form postgres://USERNAME:PWD@DBHOST/DBNAME
if [ -n "${USE_SECRET_MANAGER}" ]; then
  
  # Database username
  if [[ -z "${SECRET_NAME_DB_USER}" ]]; then
    echo "ERROR: Since USE_SECRET_MANAGER is set, SECRET_NAME_DB_USER was expected to be set"
  else
    _DB_USER=`gcloud secrets versions access latest --secret="${SECRET_NAME_DB_USER}"`
  fi

  #Database Password
  if [[ -z "${SECRET_NAME_DB_PWD}" ]]; then
    echo "ERROR: Since USE_SECRET_MANAGER is set, SECRET_NAME_DB_PWD was expected to be set"
  else
    _DB_PWD=`gcloud secrets versions access latest --secret="${SECRET_NAME_DB_PWD}"`
  fi

  # Database Name - example 'main'
  if [[ -z "${SECRET_NAME_DB_NAME}" ]]; then
    echo "ERROR: Since USE_SECRET_MANAGER is set, SECRET_NAME_DB_NAME was expected to be set"
  else
    _DB_NAME=`gcloud secrets versions access latest --secret="${SECRET_NAME_DB_NAME}"`
  fi

    if [[ -z "${SECRET_NAME_DB_HOST}" ]]; then
    echo "ERROR: Since USE_SECRET_MANAGER is set, SECRET_NAME_DB_HOST was expected to be set"
  else
    _DB_HOST=`gcloud secrets versions access latest --secret="${SECRET_NAME_DB_HOST}"`
    echo "BRUH"
    echo $_DB_HOST
  fi


  # Although at this point we can set env vars that the script below needs (user, pass etc.),
  # we are gonna create a DATABASE_URL so that we don't have to track variables name
  # if they ever change in a future revision of this original source repo.
  DATABASE_URL="postgres://${_DB_USER}:${_DB_PWD}@${_DB_HOST}/${_DB_NAME}"
  #echo ""
  #echo "Database URL created: ${DATABASE_URL}"
fi
...
################## Auto generated ##################
[databases]
${DB_NAME:-*} = host=${_DB_HOST:?"Setup pgbouncer config error! You must set DB_HOST env"} \
